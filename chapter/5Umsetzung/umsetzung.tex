...
\subsection{Parallelisierung}
    ...
    Bedingungsprüfung und der anschließenden Ausführung in einen \textit{Thread} ausgelagert, der durch einen \textit{ThreadPoolExecutor} im java Code angestoßen wird. 
    Durch das Auslagern des konkreten Prozesses in einen eigenen Thread können mehrere Regeln gleichzeitig ausgeführt werden, sofern die Bedingungsprüfung zutrifft und die 
    ...
    Minute beanspruchen, würde sich das Licht erst nach zwei Minuten einschalten. 
\subsection{Reflection} 
\subsection{Transformation}
\subsubsection*{Transformationsobjekt mittels Switch-Case-Anweisung}
\subsubsection*{Transformationsobjekt mittels eigener Klasse}
\subsection{Inverse Transformation}
Damit der Anwender des Frameworks bei der Regeldefinition einen geringen Aufwand erfährt und dennoch eine Zustandsänderung (hervorrufen) 
kann, soll eine inverse Transformation stattfinden. Dadurch soll anhand der Zustandsänderung, die der Entwickler erwartet, dementsprechend 
z.B. die zugehörige Aktion mittels \acs{MQTT}-Nachricht veröffentlicht werden. In Folge dessen muss der Entwickler bei der Implementierung einer 
Regel keine Kenntnis über die Attribute und bspw. das \acs{MQTT}-Topic haben, die für die inverse Transformation vorgesehen sind. Lediglich muss 
das richtige Attribut im Zustandsraum angesprochen und geändert werden. Ein konkretes Beispiel: 
\\
\linebreak
Wird ein Lichtschalter eingeschaltet, so wird ein \acs{MQTT}-Topic konsumiert, dass den Zustandsraum ändert, indem der Schalter aktiviert 
wurde. Daraufhin wird eine Regel ausgeführt, deren Regelaktion die LED-Leuchte  aktivieren soll. Hierfür muss erneut eine  \acs{MQTT}-Nachricht 
über ein bestimmtes Topic veröffentlicht werden. Da diese Transformation bereits definiert ist, soll  innerhalb der Regel nur die Zustandsänderung 
definiert werden können. Mithilfe der inversen Transformation wird anhand der Zustandsänderung die dementsprechende Aktion  invertiert und unter 
dem Topic eine Nachricht veröffentlicht, die die LED konsumiert und daraufhin das Licht angeschaltet wird. 
