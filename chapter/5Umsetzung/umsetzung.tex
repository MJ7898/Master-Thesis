...
\subsection{Transformation}
    ...
    erläutert und anschließend gegenübergestellt. Das dabei verfolgte Ziel, ist dem Anwender den möglichst komfortableren Weg zu bieten, eine 
    Transformation zu definiere, sodass die Aufwände für denjenigen gering bleiben.
    ...
    ausgelöst. Das konkrete Vorgehen ist dem folgenden Sequenzdiagramm, sowie dem Quellcode-Ausschnitt (siehe \ref{code:switch-case}) zu entnehmen:
    \begin{figure}[hbt!]
        \centering
...
\end{lstlisting}
    ...
    Anwender, um innerhalb einer Regel eine weitere Zustandsänderung vorzunehmen, das Topic erneut aufgreifen und in Kenntnis des jeweiligen sein. Mit dem 
    ...
    \subsubsection*{Transformationsobjekt mittels eigenem Objekt}
        Im Gegensatz zu der Verwendung einer Switch-Case-Anweisung wird ein Klasse implementiert, mit der je nach Bedarf 
        weitere Objekte mit unterschiedlichen Werten erzeugt werden kann. 
    \begin{figure}[hbt!]
        \centering
...
public class SimpleTransformer extends AbstractTransformer<InnovationLabKA> {
    public SimpleTopicWhiteListTransformation(
            String topic, String key, List<?> whiteList, String attribute) {
        setTopic(topic);
        setKey(key);
        setPayloadValues(whiteList);
        try {
            setField(InnovationLabKA.class.getDeclaredField(attribute));
...
\end{lstlisting}
     Hierbei werden die vorab definierten Transformationen des Entwicklers 
    in einer Liste gespeichert, damit das darin enthaltene Topic mit dem eingehenden verglichen werden kann. Dafür wird über die Liste iteriert und jede Transformation, sowie deren zugewiesenes Topic geprüft. Sofern es eine Übereinstimmung gibt, wird Gibt es eine Übereinstimmung, Der Ablauf kann dem Sequenzdiagramm (siehe Abbildung \ref{fig:kommunikationsequenz}) entnommen werden. 
