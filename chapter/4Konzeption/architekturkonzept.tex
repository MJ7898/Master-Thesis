...
\label{sec:architekturkonzept}
    ...
    hinzukommender Kommunikationsschnittstellen, damit in Zukunft weitere \acs{IoT}-Protokolle mit dem Framework genutzt werden können. 
    Die geplante Struktur kann dem Klassendiagramm in Abbildung (\ref{fig:patternarchitektur}) entnommen werden. 
    Der Client, in dem speziellen Fall der \textit{\acs{MQTT}-Topic-Subscriber}, wird mit einem \textit{Transformationsadapter} versehen. Hierbei werden die 
    ...
    alle für das Framework notwendigen Informationen übergibt. Dadurch bleibt die interne Arbeitsweise des Frameworks unverändert und fördert so die Erweiterbarkeit des 
    Systems.
    \begin{figure}[hbt!]
        \centering
    ...
    Prozessschicht des Frameworks aufgerufen, die daraufhin alle dem Framework bekannten Regeln überprüft und die Zutreffenden startet. Der Durchlauf wird in dem Abschnitt (\ref{subsec:logikschicht}) 
    dargestellt. 
    \begin{figure}[hbt!]
        \centering
    ...
    \\
    Darin wird die Kopie des Zustandsobjektes inklusive der Änderung vorgenommen werden, damit in folgenden Prozessen keine Inkonsistenz des Zustandsraumes entsteht. %die Repräsentation des Objektes nicht verfälscht wird. 
    ...
    und so mehrere Zustandsänderungen erfolgen könnten. Zusätzlich ist der Kopiervorgang zu sperren, dass ebenso keine Inkonsistenz des Zustandes entsteht. 
    ...
    \end{figure}
    \\
    Der initiale Ablauf von der Erstellung einer Kopie, bis hin zur Prüfung der Regelbedingung, sowie das anschließende Durchführen gestaltet sich, wie dem folgenden Sequenzdiagramm zu entnehmen ist. 
    \\
    \pagebreak
    \begin{figure}[hbt!]
        \centering
    ...
    \\
    %\linebreak
    Dem Diagramm (\ref{fig:logiksequenz}) ist ebenso die inverse Transformation (\ref{subsec:inverseTransformation}) zu entnehmen. Mithilfe dieser sollen erneute 
    ...
    Auf die konkrete Umsetzung der soeben architektonisch beschriebenen Komponenten wird in folgendem Kapitel (siehe Umsetzung \ref{chap:umsetzung}) 
    eingegangen.
    \\
